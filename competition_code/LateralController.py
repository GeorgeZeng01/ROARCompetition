...
class LatController:
    def run(self, vehicle_location, vehicle_rotation, waypoints) -> float:
        ...
        if len(waypoints) < 1:
            # Return 0 if no waypoints are available
            return 0

        # Define weights increasing linearly for the next 20 waypoints
        ...
        # Create a temporary averaged waypoint
    ...
    def calculate_steering(self, vehicle_location, vehicle_rotation, waypoint) -> float:
        ...
        Returns:
            float: Steering command.
        """
        # Extract x, y coordinates of the waypoint
